#!/usr/bin/env python3
"""Module that contains the function convolve.
"""
...
def convolve(images, kernels, padding='same', stride=(1, 1)):
    ...
    Args:
        ...
        kernels (numpy.ndarray): N-dimensional array with shape (kh, kw, c, nc)
            containing the kernels for the convolution where kh is the
            height of the kernel, kw is the width of the kernel, and nc is the
            number of kernels.
    ...
    for x in range(0, (h + (2 * pad_h) - kh + 1), sh):
        ...
        for y in range(0, (w + (2 * pad_w) - kw + 1), sw):
            for z in (range(nc)):
                output = np.sum(
                    (images[:, x: x + kh, y: y + kw, :] * (kernels[:, :, :, z])),
                    axis=(1, 2, 3)
                )
                convol[:, i, j, z] = output
            j += 1
